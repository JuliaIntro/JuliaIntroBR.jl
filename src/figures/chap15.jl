function fig15_1(output::Symbol, font::String, scale::Float64)
  ...
  \node[anchor=west] at (-1.25, 0.75) {\tt Ponto};
...
function fig15_2(output::Symbol, font::String, scale::Float64)
  p = TikzPicture(L"""
  \node[anchor=east](box) at (-2.75, 0) {\tt caixa};
  \node[draw, fill=mycolor, minimum width=4.5cm, minimum height=1.5cm](Rectangle) at(0,0){};
  \node[anchor=west] at (-1.0, 1) {\tt Retângulo};
  \node[anchor=east] (w) at(0, 0.55) {\tt comprimento};
  \node[anchor=west] (wv) at (1.0, 0.55) {\tt 100.0};
  \node[anchor=east] (h) at(0, 0) {\tt altura};
  \node[anchor=west] (hv) at (1.0, 0) {\tt 200.0};
  \node[anchor=east] (corner) at(0, -0.5) {\tt vértice};
  \draw[-latex] (box) -- (Rectangle);
  \draw[-latex] (w) -- (wv);
  \draw[-latex] (h) -- (hv);
  \node[draw, fill=mycolor, minimum width=2.5cm, minimum height=1cm](MPoint) at(4,-0.5){};
  \node[anchor=west] at (3.50, 0.25) {\tt MPonto};
