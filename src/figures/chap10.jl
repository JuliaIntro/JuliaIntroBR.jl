function fig10_1(output::Symbol, font::String, scale::Float64)
  p = TikzPicture(L"""
  \node[anchor=east](ch) at(-2.75, 0) {\tt vegetais};
  \node[draw, fill=mycolor, minimum width=3.5cm, minimum height=1.5cm](chv){};
  \node[anchor=east] (ch1) at(-1.25, 0.5) {\tt 1};
  \node[anchor=west] (chv1) at (-0.25, 0.5) {\tt "Cenoura"};
  \node[anchor=east] (ch2) at(-1.25, 0) {\tt 2};
  \node[anchor=west] (chv2) at (-0.25, 0) {\tt "Brócolis"};
  \node[anchor=east] (ch3) at(-1.25, -0.5) {\tt 3};
  \node[anchor=west] (chv3) at (-0.25, -0.5) {\tt "Alface"};
  \draw[-latex] (ch) -- (chv);
  \draw[-latex] (ch1) -- (chv1);
  \draw[-latex] (ch2) -- (chv2);
  \draw[-latex] (ch3) -- (chv3);
  \node[anchor=east](nu) at(-2.75, -1.75) {\tt números};
  ...
  \draw[-latex] (nu2) -- (nuv3);
  \node[anchor=east](em) at(-2.75, -3) {\tt vazia};
...
function fig10_5(output::Symbol, font::String, scale::Float64)
  ...
  \node[anchor=east] at(-1.2,-1){\tt deleta\_cabeça!};
