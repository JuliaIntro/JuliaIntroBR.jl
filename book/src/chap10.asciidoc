...
Este capítulo apresenta um dos tipos internos mais úteis do Julia, as listas. Você também aprenderá sobre objetos e o que pode acontecer quando você tiver mais de um nome para o mesmo objeto.


=== Uma Lista é uma Sequência

Como uma string, uma _lista_ é uma sequência de valores. Em uma string, os valores são caracteres; em um lista, pode ser de qualquer tipo. Os valores de um lista são chamados de _elementos_ ou por vezes _itens_.
(((lista)))(((elemento)))(((item)))

Existem diversas maneiras de criar uma nova lista; a mais simples é inserir os elementos entre colchetes (+[ ]+), exemplo:
(((colchetes)))
...
Uma lista com outra lista é dita _aninhada_.
(((aninhada)))
...
O tipo de lista é especificado entre as chaves e é composto pelo tipo e um número. O número indica as dimensões. A lista +vazia+ contém valores do tipo +Any+., isto é, pode ser designado a valores de todos os tipos.
...
Listas são representadas por caixas e pelos elementos das listas dentro. +vegetais+ refere-se a uma lista com três elementos indexados +1+, +2+ e +3+. +números+ contém dois elementos; o diagrama mostra que a lista do segundo elemento foi reatribuída de +1997+ para +7+. +vazia+ refere-se a uma lista sem elementos.
...
* Se você tentar ler ou escrever um elemento que não existe, você obterá um +BoundsError+.
...
Esse laço percorre a lista e atualiza cada elemento. +length+ retorna o número de elementos de uma lista. Toda vez que o laço é percorrido, +i+ obtém o índice do próximo elemento. O comando de atribuição no corpo usa +i+ para ler os valores antigos do elemento e designa ao novo valor:
...
Para somar todos os números de uma lista, podemos utilizar um laço como o seguinte:

[source,@julia-setup]
----
function somar_todos(t)
    total = 0
    for x in t
        total += x
    end
...
+total+ é iniciado em 0. Toda vez que o laço é percorrido, +pass:[+=]+ captura um elemento da lista. O operador +pass:[+=]+ fornece um jeito fácil de atuaizar uma variável. Esse _comando de atribuição aumentada_,
...
Um operador como +todas_maiúsculas+ é por vezes chamado de _mapa_ pois "mapeia" a função (neste caso +uppercase+) em cada um dos elementos em uma sequência.
(((mapa)))
...
Um operador como +apenas_maiusculas+ é chamado de _filtro_ pois seleciona alguns dos elementos e filtra outros.
(((filtro)))
...
Para cada operador binário como +pass:[^]+, existe um _operador ponto_ pass:[<code>.^</code>] correspondente que é automaticamente definido para atuar +pass:[^]+ elemento-a-elemento nas listas. Por exemplo, pass:[<code>&#91;1, 2, 3&#93; ^ 3</code>] não é definido, mas pass:[<code>&#91;1, 2, 3&#93; .^ 3</code>] é definido como calcular elemento a elemento o resultado pass:[<code>&#91;1^3, 2^3, 3^3&#93;</code>]:
...
No caso o delimitador é um caractere de espaço. Para concatenar strings sem espaços, você não precisa especificar um delimitador.


=== Objetos e Valores

Um _objeto_ é algo que uma variável pode se referir. Desde então, você poderia usar "objeto" e "valor" sem distinção.
...
Nesse caso poderíamos dizer que as duas listas são _equivalentes_, porque possuem os mesmos elementos, mas não _indênticos_, porque elas não são o mesmo objeto. Se dois objetos são idênticos, eles também são equivalentes, mas se eles são equivalentes, eles não necessariamente são idênticos.
...
Quando você passa uma lista para uma função, a função obtém uma referência para a lista. Se a função modifica a lista, quem chama nota a diferença. Por exemplo, +deleta_cabeça!+ remove o primeiro elemento de uma lista:
...
No início de +não_deleta_cabeça+, +t+ e +t3+ se referem à mesma lista. No final, +t+ se refere a uma nova lista, mas +t3+ continua a se referir à original, uma lista não modificada.
...
(((tail)))((("função", "definido pelo programador", "calda", see="calda")))
...
+
Se você precisa usar uma função como +sort!+, que modifica o argumento, você precisa manter a lista original, você pode fazer uma cópia:
(((sort!)))
+
[source,@julia-repl-test chap10]
----
...
delimitador::
Um caracter ou string utilizada para indicar onde uma string deve ser cortada.
(((delimitador)))
...
Escreva uma função chamada +soma_aninhada+ que pega uma lista de listas com números inteiros e adicione os elementos de todas as listas aninhadas. Por exemplo:
...
Escreva uma função chamada +interior!+ que pega uma lista, modifique-a removendo o primeiro e o último elementos e retorne +nothing+. Por exemplo:
(((interior!)))((("função", "definido pelo programador", "interior!", see="interior!")))
...
Escreva uma função chamada +é_ordenada+ que use uma lista como parâmetro e retorne +true+ se a lista for classificada em ordem crescente e +false+ caso contrário. Por exemplo:
...
Duas palavras são anagramas se você puder reorganizar as letras de uma para soletrar a outra. Escreva uma função chamada +é_anagrama+ que use duas strings e retorne + true + se forem anagramas.
(((é_anagrama)))((("função", "definida pelo programador", "é_anagrama", see="é_anagrama")))

[[ex10-7]]
===== Exercise 10-7

Escreva uma função chamada +tem_repetidas+ que pegue uma lista e retorne +true+ se houver algum elemento que apareça mais de uma vez. Não deve modificar a lista original.
(((tem_repetidas)))((("função", "definida pelo programador", "tem_repetidas", see="tem_repetidas")))
...
Escreva uma função chamada +é_bisseção+ que usa uma lista classificada e um valor-alvo e retorna +true+ se a palavra estiver na lista e +false+ se não estiver.
