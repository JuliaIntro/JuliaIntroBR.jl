...
Apesar da execução de algoritmos seja chata, a construção é interessante, intelectualmente desafiadora e uma parte central da ciência da computação.
...
Copie o laço de <<square_roots>> e encapsule-o em uma função chamada +minha_raiz+ que use a variável +a+ como parâmetro, e escolha um valor razoável de +x+ que retorne uma estimativa da raiz quadrada de +a+.
...
Escreva uma função chamada +avalie_laço+ que solicite iterativamente ao usuário, e então pegue a entrada recebida, avalie-na usando +eval+ e depois imprima o resultado. Ele deve continuar até o usuário digitar +concluído+ e retornar o valor da última expressão avaliada.
