...
. Se for falsa, saia do comando _while_ e continue a execução para o próximo comando.
...
Copie o laço de <<square_roots>> e encapsule-o em uma função chamada +minha_raiz+ que usa a variável +a+ como um parâmetro e escolha um valor razoável de +x+ que retorne uma estimativa da raíz quadrada de +a+.
...
A primeira coluna é um número, +a+; a segunda coluna é a raíz quadrada de +a+ calculado com +minha_raiz+; a terceira coluna é a raiz quadrada calculada por +sqrt+; a quarta coluna é o valor absoluto da diferença entre as duas estimativas.
