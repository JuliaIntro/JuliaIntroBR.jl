...
Esse programa demonstra outro padrão de programação chamado _contador_. A variável +contador+ é inicializada com 0 e incrementada toda vez que um +a+ é encontrado. Quando a função sai do loop, +contador+ contém o resultado-o número total de +a+’s.
(((contador)))
...
Na primeira vez dentro do laço, o valor de +j+ é 3, que tem que ser 4. Isso pode ser consertado movendo +j = prevind(palavra2, j)+ para o final do laço +while+.
