...
* Erros semânticos são problemas com um programa que roda sem produzir mensagens de erro, mas não faz a coisa certa. Exemplo: Uma expressão pode não ser avaliada na ordem que você espera, produzindo um resultado incorreto.
((((erro semântico)))

O primeiro passo na depuração é descobrir com qual o tipo de erro você está lidando. Embora as seguintes seções sejam organizadas pelo tipo de erro, algumas técnicas são aplicáveis em mais de uma situação.


=== Erros de sintaxe

Os erros de sintaxe são geralmente fáceis de corrigir uma vez que você descubra o que são. Infelizmente, as mensagens de erro muitas vezes não são úteis. As mensagens mais comuns são +ERROR: LoadError: syntax: incomplete: premature end of input+ e +ERROR: LoadError: syntax: unexpected "="+, que não são muito informativo.
...
Se você está construindo o programa de forma incremental, você deve ter uma boa idéia de onde o erro está. Ele estará na última linha que você adicionou.

Se você estiver copiando o código de um livro, comece comparando seu código com o código do livro com muito cuidado. Verifique todos os caracteres. Ao mesmo tempo, lembre-se de que o livro pode estar errado, portanto, se você vir algo que parece um erro de sintaxe, talvez seja.
...
. Certifique-se você tem a palavra-chave +end+ no final de cada comando composto, incluindo os blocos de +for+, +while+, +if+, e +function+.
...
. Se você tiver sequências de múltiplas linhas com aspas triplas, confira se a sequência foi finalizada corretamente. Uma sequência não terminada pode causar um erro de token inválido no final do seu programa, ou pode tratar a parte seguinte do programa como uma sttring até chegar à próxima string. No segundo caso, pode não produzir uma mensagem de erro!
...
. Se você tem caracteres que não são ASCII no código (incluindo as strings e os comentários), isso pode causar um problema, embora o Julia geralmente lide com caracteres não-ASCII. Tenha cuidado quando você colar texto de uma página da web ou de outra fonte.
...
Se a REPL diz que há um erro e você não o vê, pode ser que você e a REPL não estão olhando para o mesmo código. Verifique seu ambiente de programação para ter certeza de que o programa que você está editando é o que Julia está tentando executar.

Se o REPL diz que há um erro e você não o vê, talvez seja porque você e o REPL não estão visualizando o mesmo código. Verifique seu ambiente de programação para garantir que o programa que você está editando é o que Julia está tentando executar.
