...
//Notas de rodapé: Bugs, Prompt, Strings
//Strings: Strings são oficialmente traduzidas como "cadeias de caracteres" por razões históricas, mas comumente usamos o uso da palavra em inglês.
//To do: Traduzir os ((("")))

O ((("cientistas da computação, habilidades de", seealso="programação")))objetivo deste livro é ensinar você a pensar como um cientista da computação. Esse meio de pensamento combina algumas das melhores ferramentas da matemática, engenharia e ciências naturais. Como matemáticos, cientistas da computação usam linguagens formais para denotar ideias (especificamente cálculos). Como engenheiros, projetam coisas, montando componentes em sistemas e avaliando ganhos e perdas entre as alternativas. Como cientistas, observam o comportamento de sistemas complexos, formulam hipóteses e testam predições.
//Trade-off traduzido como ganhos e perdas"

A ((("resolução de problemas")))habilidade mais importante para um cientista da computação é a solução de problemas. Solucionar problemas significa ter habilidade para formular problemas, pensar em soluções com criatividade, e expressar a solução clara e precisa. Como se vê, o processo de aprendizagem para programar é uma excelente oportunidade de praticar a habilidade de resolução de problemas. Por essa razão esse capítulo tem como título "O caminho da programação". À princípio, você estará aprendendo a programar, uma habilidade muito útil à você. Após, você usará a programação como um significado para o fim. Na medida que formos evoluindo, esse fim ficará mais claro.
...
Os ((("instruções", seealso="sentenças"))) detalhes são diferentes em diferentes linguagens de programação, mas algumas instruções básicas aparecem em praticamente em todos os idiomas:
...
Um ((("Julia", "executando")))dos desafios de começar com o Julia é que talvez você precise instalá-lo a um software relacionado ao seu computador. Se você está familiarizado com o seu sistema operacional, e especificamente se você está confortáel com a interface das linhas de comando, você não terá problemas em instalar o Julia. Mas, para iniciantes, pode ser assustador aprender sobre administração de sistemas operacionais e programação ao mesmo tempo.
...
No navegador, você pode executar o https://www.juliabox.com[JuliaBox]. Nenhuma instalação é necessária - Apenas escolha um navegador, faça o _log in_ e comece a programar (veja <<juliabox>>).

O Julia _REPL_ (Read-Eval-Print Loop, que traduzido ao pé da letra, significa Ler, Avaliar e Repetição de impressão) é um programa que lê e executa os códigos do Julia. Você pode começar o REPL abrindo o terminal no JuliaBox e escrevendo *+julia+* na linha de comando. Quando executar, você deverá ver uma saída como essa:
...
As primeiras contém informações sobre o REPL, que deve ser diferente para você. Porém, você deve checar que o número da versão seja pelo menos 1.0.0.

A última linha é um _prompt_ que indica que o REPL está pronto para você inserir o código. Se você digitar uma linha de código e pressionar Enter, o REPL exibirá o seguinte resultado:
...
Este ((("declaração de impressão", "função println")))((("saída", "declaração de impressão")))é um exemplo de uma declaração de impressão, embora não imprima nada no papel. Ele exibe o resultado na tela.

As aspas no programa marcam o início e o final do texto a ser exibido; eles não aparecem no resultado.

Os parênteses indicam que +printl+ é uma função. Nós chegaremos em funções no <<Capítulo 3>>. ((("<<chap03>>"")))
...
O operador ((("$$/$$ (operador de divisão)", primary-sortas="* divisão")))((("divisão", "operador de divisão ($$/$$)"))) +/+ executa divisão:
...
Finalmente, o operador ((("$$^$$ (caret)", "operador de exponenciação", primary-sortas="* circunflexo")))((("operador de exponenciação ($$^$$)")))((("circunflexo ($$^$$)", "operador de exponenciação")))+^+ executa exponenciação; isto é, aumenta um número a uma potência:

//easter egg de mochilheiro das galáxias, certeza
[source,@julia-repl-test]
----
julia> 6^2 + 6
42
...
Inteiros pertencem ao tipo +Int64+, strings pertencem a +String+, e pontos flutuantes pertencem a +Float54+.

E ((("aspas (&quot;&#x2026;&quot;)", "anexo de strings")))((("&quot;&#x2026;&quot; (aspas)", "anexo de strings", primary-sortas="* aspas")))quanto a valores como +"2"+ e +"42.0"+? Eles parecem números, mas estão entre parênteses como strings. Eles também são strings:
...
//This is not a legal integer in Julia, but it is legal: --> Como traduzir melhor?
[source,@julia-repl-test]
----
julia> 1,000,000
(1, 0, 0)
...
_Linguagens Naturais_ ((("linguagem natural", id="ch1nat", range="startofrange"))) são aquelas linguagens que as pessoas falam, como Inglês, Espanhol, Português e o Francês. Eles não são projetados por pessoas (embora pessoas tentem impor-lhes alguma ordem); eles evoluíram naturalmente.
...
Regras ((("tokens", id="ch1nat5", range="startofrange")))((("estrutura", id="ch1nat6", range="startofrange")))de sintaxe são fornecidas em dois tipos, pertencentes a _tokens_ e _estrutura_. Tokens são elementos básicos da linguagem, como palavras, números e elementos químicos. Um dos problemas com latexmath:[\(3 += 3 \$ 6\)] é que latexmath:[\(\$\)] não é um token permitido em matemática (pelo menos até onde eu sei). Da mesma forma, latexmath:[\(\mathrm{_2Zz}\)] não é permitido pois não há elemento com abreviação latexmath:[\(\mathrm{Zz}\)].

O segundo tipo de regra de sintaxe pertence ao caminho que tokens são combinados. A equação latexmath:[\(3+=3\)] não é permitida porque mesmo que latexmath:[\(+\)] e latexmath:[\(=\)] sejam tokens permitidos, você não pode ter um logo após o outro. Da mesma forma, em uma fórmula química, o subscrito vem depois do nome do elemento, e não antes.

//This is @ well-structured Engli$h sentence with invalid t*kens in it. This sentence all valid tokens has, but invalid structure with.
//Optei por adaptar ao portugues da seguinte forma:

Essa é um@ sentença bem estruturada em portuguê$ com t*kens inválidos. Essa frase possui todos os tokens válidos, mas com estrutura invalida.

Quando ((("análise"))) você lê uma sentença em Poruguês ou uma declaração numa linguagem formal, você tem que descrever a estrutura (apesar de que em uma linguagem natural você faz isso subconscientemente). Esse processo é chaamdo de _análise_.

Embora as linguagens formais e naturais tenham muito recursos em comuns — tokens, estruturas, e sintaxes —  existem algumas diferenças:

Ambiguidade::
Linguagens naturais são cheias de ambiguidades, com pessoas acordando com elas e usando em pistas contextuais e outras informações. Linguagens formais são designadas para serem próximas e completamente desambígua sem ambiguidade, o que significa que toda declaração possui exatamente um significado, independentemente do contexto.

Redundância::
Para compensar a ambiguidade e reduzir os mal-entendidos, as línguas naturais empregam muita redundância. Como resultado, eles geralmente são detalhados. Linguagens formais são menos redundantes e mais concisas.

Literalidade::
As línguas naturais estão cheias de expressões idiomáticas e metáforas. Se eu disser: "O centavo caiu", provavelmente não há centavo e nada está caindo (nesse idioma significa que alguém entendeu alguma coisa após um período de confusão). Linguagens formais significam exatamente o que dizem.

Como todos nós crescemos falando línguas naturais, às vezes é difícil nos adaptarmos às línguas formais. A diferença entre linguagem formal e natural é como a diferença entre poesia e prosa, mas mais ainda:
...
Programas::
O significado de um programa de computador é inequívoco e literal e pode ser entendido inteiramente pela análise dos tokens e da estrutura.

As linguagens formais são mais densas que as linguagens naturais, por isso leva mais tempo para lê-las. Além disso, a estrutura é importante, portanto nem sempre é melhor ler de cima para baixo, da esquerda para a direita. Em vez disso, você aprenderá a analisar o programa em sua cabeça, identificando os tokens e interpretando a estrutura. Finalmente, os detalhes são importantes. Pequenos erros de ortografia e pontuação, com os quais você pode se dar bem em idiomas naturais, podem fazer uma grande diferença na linguagem formal.
(((range="endofrange", startref="ch1nat")))(((range="endofrange", startref="ch1nat2")))(((range="endofrange", startref="ch1nat3")))(((range="endofrange", startref="ch1nat4")))(((range="endofrange", startref="ch1nat5")))(((range="endofrange", startref="ch1nat6")))


=== Depuração

Programadores ((("erros (bugs)")))((("depuração", "emoções de, lidando com")))cometem erros. Por motivos caprichosos, erros de programação são chamados de _bugs_ e o processo de rastrea-los é chamado de _depuração_.
...
Aprender a depurar pode ser frustrante, mas é uma habilidade valiosa que é útil para muitas atividades além da programação. No final de cada capítulo, há uma seção, como esta, com minhas sugestões para depuração. Espero que eles ajudem!
...
inteiros::
Um ((("tipo inteiro (Int64)")))tipo de que rerpesenta todos os números.
...
linguagem formal::
Qualquer ((("linguagem formal"))) uma das linguagens que as pessoas foram designadas para propósitos específicos, como representar ideias matematicas ou programas de computador. Todas ((("programming language"))) as linguagens de programação são formais.

sintaxe::
As ((("sintaxe"))) regras que governam a estrutura de um programa.

token::
Um ((("tokens")))dos elementos mais básicos de uma estrutura de sintaxes de um programa, análogo a uma palavra numa linguagem natural.

estrutura::
O ((("estrutura")))caminho que os tokens são combinados.

análise::
Para ((("análise"))) examinar um programa e analisar uma estrutra de sintaxe.

bug::
Um ((("erros (bugs)", seealso="depuração")))((("bugs", see="errors"))) erro de um programa.

depurar::
O ((("depuração", seealso="erros (bugs); testes"))) processo de encontrar e corrigir bugs.
...
[[ex01-1]]
===== Exercício 1-1
Sempre que você estiver experimentando um novo recurso, tente cometer erros. Por exemplo, no "Olá, mundo!" programa, o que acontece se você deixar de fora uma das aspas? E se você deixar de fora os dois? E se você soletrar +println+ errado?
...
. Em uma declaração impressa, o que acontece se você deixar de fora um dos parênteses ou ambos?
...
. Em noração matemática, zeros à esquerda estão corretos, como em +02+. O que acontece se você tentar isso em Julia?
...
. Quantos segundos existem em 42 minutos 42 segundos?
