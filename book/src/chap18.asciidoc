...
No capítulo anterior, introduzimos o mecanismo de despacho múltiplo e o polimorfismo. A não especificação do tipo dos argumentos resulta em um método que pode ser chamado com argumentos de qualquer tipo. A especificação de um subconjunto de tipos permitidos na assinatura de um método é um passo lógico seguinte.
...
Se você não joga pôquer, pode ler sobre isso em https://pt.wikipedia.org/wiki/P%C3%B4quer, mas não precisa pois vou lhe dizer o que você tem que saber para os exercícios.
...
Se queremos definir um novo objeto para representar uma carta de baralho, é óbvio quais devem ser os atributos: o valor e o naipe. Mas não é tão óbvio que tipo de atributo deve ser. Uma possibilidade é usar strings contendo palavras como +"Estadas"+ para os naipes e +"Rainha"+ para os valores. Um problema com essa implementação é que não seria fácil comparar as cartas para ver quais possuíam maior valor ou naipe.

Uma alternativa é usar números inteiros para _codificar_ os valores e os naipes. Nesse contexto, "codificar" significa que vamos definir um mapeamento entre números e naipes, ou entre números e valores do baralho. Esse tipo de codificação não pretende ser um segredo (isso seria "criptografia").
(((codificar)))

Por exemplo, esta tabela mostra a correspondência dos naipes com os números inteiros:
...
Este código facilita a comparação das cartas; já que os naipes mais valorizados estão relacionados com números mais altos, podemos comparar naipes comparando seus números.

Estou usando o símbolo latexmath:[\(\mapsto\)] para deixar claro que essas correspondências não fazem parte do programa Julia. Eles fazem parte da formulação do programa, mas não aparecem explicitamente no código.
...
struct Carta
    suit :: Int64
    rank :: Int64
    function Carta(suit::Int64, rank::Int64)
...
Para criar uma +Carta+, você chama +Carta+ com o naipe e o valor da carta que você deseja:
