...
Va até o Projeto Gutenberg (https://guttenberg.org) e baixe seu livro favorito de domínio público em formato de texto sem formatação.
(((Projeto Gutenberg)))
...
A função +rand+ retorna um ponto flutuante aleatório entre +0.0+ e +1.0+ (incluindo 0.0 mas não 1.0). Toda vez que você usa o comando +rand+, você obtém o próximo número de uma longa série. Para ver uma amostra disso, execute este laço:
(((rand)))
...
Para encontrar as palavras mais frequentes, podemos fazer uma lista de tuplas, onde cada tupla contém uma palavra e a sua frequência e fazemos a ordenação. A função seguinte recebe um histograma e retorna uma lista de tuplas que contém a frequência de palavras.
(((mais_comum)))((("função", "definida pelo programador", "mais_comum", see="mais_comum")))(((reverse)))(((sort)))
...
o argumento +num+ fica com o valor do argumento. Em outras palavras, o valor opcional _sobrepõe_ o valor padrão.
(((sobreposisão)))
...
Um jeito de medir essa relação é através da análise de Markov, que caracteriza, para uma sequência de palavras dadas, a probabilidade das palavras que possam vir a seguir. Por exemplo, a música _Epitáfio_ (do Titãs) tem o seguinte trecho:
(((análise de Markov)))
...
No texto, o termo “devia” é sempre seguido da palavra “ter”, mas a palavra “ter” pode ser seguida de “amado” ou “arriscado”.

O resultado da análise de Markov é um mapeamento de cada prefixo (como “devia” e “ter”) a todos os possíveis sufixos (como “amado” ou “arriscado”).
(((prefixo)))(((sufixo)))
...
Por exemplo, se você começar com o prefixo “devia“, então a póxima palavra deverá ser “ter”, pois é o prefixo que aparece apenas uma vez no texto. O próximo prefixo é “devia ter”, então o próximo sufixo poderá ser “amado” ou “arriscado”.
...
Como você deve escolher? O primeiro passo é pensar nas operações que você precisará implementar para cada estrutura de dados. Para os prefixos, precisamos remover palavras do começo e adicionar ao final. Por exemplo, se o prefixo atual é "devia" e a próxima palavra é "ter", você precisa formar o próximo prefixo, "devia ter".
...
Adicionar um novo sufixo é igualmente fácil para a implementação da lista ou do histograma. Escolher um elemento aleatório de uma lista é fácil; escolher de um histograma é mais difícil de ser feito eficientemente (veja <<ex13-7>>).

Até agora, conversamos principalmente sobre a facilidade de implementação, mas há outros fatores a serem considerados na escolha das estruturas de dados. Um deles é o tempo de execução. Às vezes, existe uma razão teórica para esperar que uma estrutura de dados seja mais rápida que outra; por exemplo, mencionamos que o operador +in+ é mais rápido paraos dicionários do que para as listas, pelo menos quando o número de elementos é grande.
...
Se você explicar o problema para outra pessoa, às vezes encontrará a resposta antes de terminar de fazer a pergunta. Muitas vezes você não precisa da outra pessoa, poderia apenas conversar com pato de borracha. E essa é a origem da estratégia conhecida chamada depuração com o pato de borracha. Não estamos inventando isso, veja https://pt.wikipedia.org/wiki/Debug_com_Pato_de_Borracha.
...
determinístico::
Referente a um programa que faz a mesma coisa toda vez que é executado, dada a mesma entrada.
(((determinístico)))
...
benchmarking::
O processo de seleção das estruturas de dados ao implementar as alternativas e testá-las em uma amostra com as entradas possíveis.
(((benchmarking)))
...
A “classificação” de uma palavra é a sua posição em uma lista de palavras ordenada pela frequência: a palavra mais comum tem classificação 1, a segunda mais comum tem classificação 2, etc.
...
Escreva um programa que leia um texto em um arquivo, conta as frequências das palavras e exiba uma linha para cada palavra, em ordem decrescente da frequência, com latexmath:[\(\log f\)] e latexmath:[\(\log r\)].
