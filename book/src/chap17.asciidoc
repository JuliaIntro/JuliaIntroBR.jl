...
Em <<chap16>>, nós definimos uma struct chamada +MeuTempo+ e em <<time>>, você escreveu uma função chamada +imprime_tempo+:
...
Para chamar esta função, você deve passar um objeto +MeuTempo+ como um argumento:
...
Para adicionar um _método_ à função +imprime_tempo+ que apenas aceita um objeto +MeuTempo+ como argumento, tudo que precisamos fazer é acrescentar +::+ seguido de +MeuTempo+ no argumento +tempo+ na definição de função:
(((método)))
...
Se você chamar a função +imprime_tempo+ com um objeto diferente de +MeuTempo+, agora recebe-se:
...
Reescreva +tempo_para_int+ e +int_para_tempo+ (de <<prototyping_versus_planning>>) para especificar os seus argumentos.
...
ERROR: MethodError: no method matching incrementar(::Int64, ::MeuTempo)
----

A assinatura do método é +incrementar(tempo::MeuTempo, segundos::Int64)+ e não +incrementar(segundos::Int64, tempo::MeuTempo)+.

Reescrever +é_depois+ para operar somente com objetos +MeuTempo+ é fácil:
(((é_depois)))
...
Um _construtor_ é uma função especial chamada para criar um objeto. Os métodos construtores padrões de +MeuTempo+ tem a seguinte assinatura:
(((construtor)))
...
+show+ é uma função especial que retorna uma representação de string de um objeto. Por exemplo, aqui está um método +show+ para os objetos +MeuTempo+:
(((show)))
...
Ao definir os métodos operadores, você pode especificar o comportamento dos operadores em tipos definidos pelo programador. Por exemplo, ao definir um método chamado +pass:[+]+ com dois argumentos +MeuTempo+, você pode usar o operador +pass:[+]+ em objetos +MeuTempo+.
...
Quando você aplica o operador +pass:[+]+ aos objetos +MeuTempo+, o Julia invoca o novo método adicionado. Quando o REPL mostra o resultado, o Julia invoca +show+. Então muita coisa acontece por trás das cortinas!
...
Na seção anterior, nós adicionamos dois objetos +MeuTempo+, mas você também pode adicionar um inteiro ao objeto +MeuTempo+:
...
Já que um método +pass:[+]+ é fornecido para os objetos +MeuTempo+, eles funcionam com +sum+:
...
Por exemplo, neste capítulo nós desenvolvemos uma struct que representa um horário do dia. Métodos que possuem argumentos indicados com este tipo incluem +tempo_para_int+, +é_depois+ e +pass:[+]+.
...
Neste exemplo, a função +imprime_tempo+ tem 2 métodos: um com o argumento +MeuTempo+ e um com o argumento +Any+.
...
Mude os campos de +MeuTempo+ para ter apenas um único campo representando os segundos passados após a meia-noite. Em seguida modifique os métodos definidos neste capítulo para funcionar com a nova implementação.
