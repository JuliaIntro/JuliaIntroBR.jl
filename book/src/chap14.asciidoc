...
Uma alternativa é armazenar o estado do programa em um banco de dados. Neste capítulo eu também irei apresentar como usar um banco de dados simples.
...
Quando você acabar de escrever no arquivo, ele deve ser fechado.
(((close)))
...
O argumeto da função write precisa ser uma string, então se nós quisermos colocar outros valores no arquivo, nós temos que convertê-los para string. A maneira mais fácil de fazer isso é com +string+ ou interpolação de strings:
...
Um path que começa com +/+ não depende do diretório atual; ele é chamado de _path absoluto_. Para achar um path absoluto para um arquivo, você pode usar +abspath+:
...
Para evitar estes erros, você pode usar funções como +ispath+ e +isfile+, mas levaria muito tempo e código para verificar todas as possibilidades.
...
O Julia inicia executando a cláusula +try+. Se tudo der certo, ele pula a cláusula +catch+ e segue adiante. Se uma exceção ocorre, ele pula fora da cláusula +try+ e executa a cláusula +catch+.
...
A limitação do +GDBM+ é de que as chaves e valores precisam ser strings ou arrays de bytes. Se você tentar usar qualquer outro tipo, você recebe um erro.
