...
O laço começa em +i=1+ e +j=nextind(palavra, 1)+ e termina quando +j> sizeof(palavra)+. A cada iteração no laço, ele compara o ++i++-ésimo caracter (que você pode pensar como sendo o caracter atual) com o ++j++-ésimo caracter (que você pode pensar como o próximo).

Se a posição do próximo caracter é alfabeticamente antecedente à posição do caracter atual, descobrimos uma quebra na tendência alfabética e retornamos +false+.
