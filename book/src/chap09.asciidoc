...
Este arquivo está em texto sem formatação, portanto, pode ser aberto com um editor de texto, mas também pode ser lido em Julia. A função interna +open+ precisa receber o nome do arquivo como parâmetro e retorna um _objeto de fluxo de arquivo_ usado para a leitura do arquivo.
...
+arquivo_entrada+ é um objeto de fluxo de arquivo usado para a entrada de dados e quando não for mais necessário, deve ser fechado com +close(fin)+.
...
O laço +for+ percorre os caracteres das palavras. Se encontrarmos a letra +'e'+, podemos retornar imediatamente +false+; caso contrário, temos que avançar para a próxima letra. Se sairmos do laço do modo convencional, isso significa que não encontramos um +'e'+, então retornamos +true+.
...
+usa_apenas+ é parecido, exceto que o sentido da condição se inverte:
(((usa_apenas)))

[source,@julia-setup chap09]
----
function usa_apenas(palavra, válido)
    for letra in palavra
        if letra ∉ válido
            return false
        end
...
function usa_todas(palavra, obrigatórias)
    usa_apenas(obrigatória, palavra)
end
----
...
Escrevi as funções da seção anterior com laços +for+ porque só precisava dos caracteres nas strings, sem precisar mexer com os índices.
...
Outra opção é usar um laço +while+:
(((declaração while)))
...
Se chegarmos ao final do laço sem uma falha, a palavra passa no teste. Para se convencer de que o laço termina corretamente, considere a palavra +"acenos"+ como um exemplo.
...
Ou podemos fazer a redução a um problema resolvido anteriormente e escrever:

[source,@julia-setup chap08]
----
function é_palíndromo(palavra)
    isreverse(palavra, palavra)
end
----

usando +isreverse+ de <<deb08>>.
(((isreverse)))
...
Selecionando +sem_e+ como exemplo, há dois casos óbvios a serem testados: palavras com +'e'+ devem retornar +false+ e palavras sem +'e'+ devem retornar +true+. Você não deve ter dificuldades para encontrar um caso de cada.

Dentro de cada caso, existem subcasos menos óbvios. Entre as palavras que possuem um "e", você deve testar as palavras com "e" no início, no final e em algum lugar no meio. Deve-se testar palavras longas, curtas e muito curtas, como a string vazia. A string vazia é um exemplo de um _caso especial_, que é um dos casos não óbvios onde os erros geralmente se ocultam.
(((caso especial)))

Além dos casos de teste gerados, você também pode testar seu programa com uma lista de palavras como +palavras.txt+. Ao avaliar a saída, pode-se detectar erros, mas tenha cuidado: você pode detectar um tipo de erro (palavras que não devem ser incluídas, mas são) e não o outro tipo
...
Uma maneira de resolver um problema, expressando-o como um caso de um problema resolvido anteriormente.
(((redução a um problema previamente resolvido)))
...
[quote]
____
Diga-me uma palavra com três letras duplas consecutivas. Darei a você algumas palavras que quase se qualificam, mas não acontece. Por exemplo, a palavra committee (comitê em inglês), c-o-m-m-i-t-t-e-e. Seria ótimo, exceto pelo 'i' que está infiltrado na palavra. Ou Mississippi: M-i-s-s-i-s-s-i-p-p-i. Se você pudesse tirar aqueles i's, funcionaria. Mas há uma palavra que possui três pares consecutivos de letras e, pelo que sei, essa pode ser a única palavra. Claro que há provavelmente mais 500, mas só consigo pensar em uma. Qual é a palavra?
____
...
[quote]
____
Eu estava dirigindo na estrada outro dia e notei meu odômetro. Conforme a maioria dos odômetros, ele mostra seis dígitos, em milhas inteiras. Por exemplo, se meu carro tivesse percorrido 300.000 milhas, então eu veria 3-0-0-0-0-0.
...
Uma milha depois, os últimos 5 números se tornaram um palíndromo. Por exemplo, poderia ter lido 3-6-5-4-5-6. Uma milha depois disso, os 4 dos 6 números do meio eram palíndromos. E você está pronto para isso? Uma milha depois, todos os 6 eram palíndromos!

A pergunta é, qual o número que estava no odômetro quando olhei pela primeira vez?
____

Escreva um programa Julia que teste todos os números de seis dígitos e mostre qualquer número que atende esses requisitos.

[[ex09-9]]
===== Exercício 9-9

Aqui está outro desafio _Car Talk_ que você pode resolver com uma busca (https://www.cartalk.com/puzzler/browse):

[quote]
____
Recentemente, visitei minha mãe e percebemos que os dois dígitos que compõem minha idade quando trocados resultavam em sua idade. Por exemplo, se ela tem 73 anos, tenho 37 anos. Imaginávamos com que frequência isso acontecia ao longo dos anos, mas nos desviamos de outros tópicos e nunca chegamos a uma resposta.

Quando cheguei em casa, descobri que os dígitos de nossas idades foram trocados seis vezes até agora. Também descobri que, se tivermos sorte, isso acontecerá novamente em alguns anos, e se tivermos muita sorte, isso acontecerá mais uma vez depois disso. Em outras palavras, isso teria acontecido 8 vezes. Então a pergunta é: qual a minha idade agora?
____

Escreva um programa em Julia que procure as soluções para este desafio.
