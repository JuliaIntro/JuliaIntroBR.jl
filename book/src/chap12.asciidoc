...
O resultado é um _objeto do tipo zip_ que sabe como iterar através dos pares. O uso mais comum de +zip+ ocorre em um laço +for+:
(((objeto do tipo zip)))(((declaração for)))
...
(((hasmatch)))((("função", "Base", "hasmatch", see="hasmatch")))

Cada vez que o loop é selecionado, Julia seleciona a próxima tupla na matriz e atribui os elementos à letra e ao número. Os parênteses em torno de + (letra, número) + são obrigatórios.
(((parênteses)))

Cada vez através do loop, Julia seleciona o próximo tuple no array e atribui os elementos à letra e ao número. Os parênteses em torno de +(letra, número)+ são obrigatórios.
(((parênteses)+))
...
Se você combinar +zip+, +for+ e tuple, você obtém uma linguagem útil para atravessar duas (ou mais) seqüências ao mesmo tempo. Por exemplo, +hasmatch+ toma duas seqüências, +t1+ e +t2+, e retorna +true+ se houver um índice +i+ tal que +t1[i] == t2[i]+:
(((hasmatch)))((("função", "Base", "hasmatch", veja="hasmatch"))))

Se você combinar + zip +, + para + e atribuição de tupla, obterá um idioma útil para percorrer duas (ou mais) seqüências ao mesmo tempo. Por exemplo, + hasmatch + usa duas seqüências, + t1 + e + t2 +, e retorna + true + se houver um índice + i + tal que + t1 [i] == t2 [i] +:
(((hasmatch))) ((("função", "Base", "hasmatch", consulte = "hasmatch"))))
...
(((mostfrequent)))((("função", "definido pelo programador", "mostfrequent", see="mostfrequent")))
